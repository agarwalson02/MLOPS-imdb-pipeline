...
def load_data(data_url:str)->pd.DataFrame:
    ...
    try:
        ...
        logging.info('Data retrieved from %s',data_url)
        return df
    except pd.errors.ParserError as e:
        logging.error('Failed to parse the csv file %s',e)
        raise
    except Exception as e:
        logging.error('Unexpected error wile loading the data %s',e)
        raise
...
def preprocess_data(df:pd.DataFrame)->pd.DataFrame:
    ...
    try:
        ...
        logging.error('Missing coloumn in the dataframe:%s',e)
        raise
    except Exception as e:
...
def save_data(train_data:pd.DataFrame,test_data:pd.DataFrame,data_path:str)->None:
    """Save the trained and test data"""
    try:
...
def main():
    try:
        ...
        df = load_data(data_url=os.path.join('notebooks', 'data.csv'))
